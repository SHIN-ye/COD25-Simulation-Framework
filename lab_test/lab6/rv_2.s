...
L51: # Test 9-1: lw load-use
    # prepare data
    addi        x20,  x0, 0
    lui         x20, 0x10010     # base ptr x20 = 0x10010000
    addi        x21,  x0, 0
    lui         x21, 0x12345
    addi        x21, x21, 0x678  # x21 = 0x12345678
    sw          x21, 0(x20)      # Mem[0] = 0x12345678
    # testing
    lw          x22, 0(x20)      # load word x22 = 0x12345678
    addi        x23, x22, 0x111  # use -> x23 = 0x12345789
    addi        x24,  x0, 0
    lui         x24, 0x12345
    addi        x24, x24, 0x789  # make expected = 0x12345789
    bne         x23, x24, fail
    j           L52

L52: # Test 9-2: lh load-use
    lh          x25, 0(x20)      # load half x25 = 0x00005678
    addi        x26, x25, 0x111  # use -> x26 = 0x5789
    addi        x27,  x0, 0
    lui         x27, 0x5         # lui : 0x5000
    addi        x27, x27, 0x789  # 0x5000 + 0x789 = 0x5789
    bne         x26, x27, fail
    j           L53

L53: # Test 9-3: lb load-use
    lb          x8, 0(x20)       # load byte x8 = 0x00000078
    addi        x9, x8, 0x11     # use -> 0x78 + 0x11 = 0x89
    addi        x10, x0, 0x89
    bne         x9, x10, fail
    j           L54

L54: # Test 10-1: Store-Use Forwarding
    addi        x21, x0, 0x456
    sw          x21, 8(x20)      
    lw          x22, 8(x20)      
    bne         x21, x22, fail
...
L55: # Test 10-2: Load-ALU-Use
    # Load and Use separated by alu instruction, do not need stall.
    addi        x21, x0, 0x345
    sw          x21, 12(x20)
    lw          x22, 12(x20)
    addi        x25, x0, 0x111
    add         x23, x22, x25
    addi        x24, x0, 0x456   # 0x345 + 0x111 = 0x456
    bne         x23, x24, fail
    j           win
